...
def get_pets(
    ...
    status_filter: Optional[str] = "?¨åº«",
    publish_status_filter: Optional[str] = "ä¸Šæ¶ä¸?,
...
def get_pet(pet_code: str, db: Session = Depends(get_db)):
    ...
    if pet is None:
        raise HTTPException(status_code=404, detail="?¾ä??°è©²æ´»é?è³‡æ?")
    return pet
...
def update_pet(
    ...
    if current_user.role == "store_manager" and current_user.store_id != db_pet.store_id:
        raise HTTPException(status_code=403, detail="?¨ç„¡æ¬Šä¿®?¹å…¶ä»–é?å¸‚ç?æ´»é?è³‡æ?")
    ...
    if pet_update.features:
        ...
        if char_count < 50 or char_count > 100:
            raise HTTPException(
                ...
                detail="?‹é??¹è‰²èªªæ?å¿…é???50 ??100 å­—ä??“ï?"
            )
...
async def upload_photo(
    ...
    if not SUPABASE_URL or not SUPABASE_KEY:
        raise HTTPException(status_code=500, detail="ç³»çµ±?ªè¨­å®?Supabase Storage ?‘è? (SUPABASE_URL, SUPABASE_KEY)ï¼Œè??¯ç¹«ç®¡ç??¡ã€?)

    supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)

    ext = os.path.splitext(file.filename)[1]
    if ext.lower() not in [".jpg", ".jpeg", ".png", ".webp", ".heic"]:
        raise HTTPException(status_code=400, detail="ä¸æ”¯?´ç?æª”æ??¼å?ï¼Œè?ä¸Šå‚³ JPG, PNG ??WEBP??)
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"ä¸Šå‚³?–ç?å¤±æ?: {str(e)}")
...
async def import_erp_csv(
    ...
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="?ªæ?ç³»çµ±ç®¡ç??¡å¯ä»¥åŸ·è¡?ERP è³‡æ??Œæ­¥")
